      ...
      */
      ...
      TEST_F (json_pointer_test, parse)
      {
        ...
        TEST_IT_START
          ...
          catch (format::json::json_pointer_error & pe)
            {
              this->_errorc[ACTUAL]++;
      ...
      TEST_F (json_pointer_test, point)
      {
        ...
        TEST_IT_START
            ...
            format::json::value & v = jp._point (j, jp._json_pointer.cbegin ());

            ASSERT_EQ ((*it).type, v.type ());

            if (v.type () == value::number_t)
              {
                format::json::number & n = static_cast<format::json::number &> (v);
      ...
      TEST_F (json_pointer_test, value)
      {
        format::json::json j (  L"{ \"foo\": [\"bar\", \"baz\"],\
                          \"\": 0,\
                          \"a/b\": 1,\
